...
def create_vertical_stacked_bar_chart(data, condition, title):
    ...
    for pitch_type, pct, color in zip(pitch_labels, pitch_percentages, pitch_colors):
        bar = ax.bar(x=0, height=pct, width=0.4, color=color, edgecolor="black", bottom=cumulative_height, label=pitch_type)
...
def create_location_plot(data):
    ...
    plt.title("Full Location", fontsize=12, weight="bold")
    plt.xlabel("Plate Loc Side", fontsize=8)
    plt.ylabel("Plate Loc Height", fontsize=8)
    kzone_x = [0.8, 0.8, -0.8, -0.8, 0.8]
    kzone_y = [3.575, 1.575, 1.575, 3.575, 3.575]
    shadow_zone_x = [1, 1, -1, -1, 1]
    shadow_zone_y = [3.775, 1.375, 1.375, 3.775, 3.775]
    plt.fill(shadow_zone_x, shadow_zone_y, color="lightgray", alpha=0.5)
    plt.plot(kzone_x, kzone_y, color="black", linewidth=1.5)
    plt.axvline(x=0.0, color="black", linewidth=1, linestyle="-")
    plt.axhline(y=2.5, color="black", linewidth=1, linestyle="-")

    for _, row in data.iterrows():
        pitch_color = pitch_type_colors.get(map_pitch_type(row['TaggedPitchType']), "gray")
        plt.scatter(row['PlateLocSide'], row['PlateLocHeight'], color=pitch_color, edgecolor="black", s=40)
